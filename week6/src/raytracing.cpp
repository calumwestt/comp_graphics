...
bool is_shadow(RayTriangleIntersection intersection, std::vector<ModelTriangle> triangles) {	
	...
	for(int i = 0; i < triangles.size(); i++) {
		...
		if((u >= 0.0) && (u <= 1.0) && (v >= 0.0) && (v <= 1.0) && (u + v <= 1.0)) {
			if(t < glm::length(shadow_ray) && t > 0.01 && i != intersection.triangleIndex) return true;
		}
	}
	return false;
}
	...
	for(int x = 0; x < window.width; x++) {
		for(int y = 0; y < window.height; y++) {
			...
			if(!isinf(intersection.distanceFromCamera)) {
				...
				uint32_t tri_colour = (255 << 24) + (int(colour.red) << 16) + (int(colour.green) << 8) + int(colour.blue);
				uint32_t shadow_colour = (255 << 24) + (int(colour.red/3) << 16) + (int(colour.green/3) << 8) + int(colour.blue/3);
				if(is_shadow(intersection, triangles)) window.setPixelColour(x, y, shadow_colour);
				else window.setPixelColour(x, y, tri_colour);
			}
		}
	}
}
...
int main(int argc, char *argv[]) {
	...
	while (true) {
		...
		//if update is commented out, orbit and look at, will not run.
		//update(window);
		draw(window);
